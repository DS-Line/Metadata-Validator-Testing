...
def load_schema(schema_path: str) -> Dict[str, List[str]]:
    ...
    schema = schema[keys[0]] if len(keys) == 1 else schema
...
if __name__ == "__main__":
    ...
    print("📂 Enter relative paths like 'assets/schema/movies.yml' or 'assets/semantics/movies.yml'")
    
    user_schema_path = input("Enter path to schema file (press Enter for default): ").strip()
    user_semantic_path = input("Enter path to semantic file (press Enter for default): ").strip()

    schema_path = user_schema_path or "./assets/schema/movies.yml"
    semantic_file = user_semantic_path or "./assets/semantics/movies.yml"

    # Convert to absolute paths
    schema_path = os.path.abspath(schema_path)
    semantic_file = os.path.abspath(semantic_file)
    ...
    try:
        print(f"\n📄 Processing file: {semantic_file}")
        
        with open(semantic_file, 'r') as f:
            file_content = f.read()
            if not file_content.strip():
                print(f"❌ EmptyFileError: {semantic_file} is empty or only contains whitespace")
            else:
                try:
                    data = yaml.safe_load(file_content)
                except yaml.YAMLError as e:
                    print(f"🔴 InvalidFormatError: {semantic_file} - {e}")
                else:
                    try:
                        ...
                        try:
                            print(f"🧪 Validating {semantic_file}")
                            validated = semantics_validator.model_validate(data)
                            print(f"✅ Passed: {semantic_file}")
                        except ValidationError as e:
                            print(f"❌ Validation Failed: {semantic_file}")
                            for err in e.errors():
                                custom_error = get_custom_error_type(err)
                                print(f"🔴 {custom_error}: {'.'.join(str(i) for i in err['loc'])} - {err['msg']}")
    except FileNotFoundError:
        print(f"❌ FileNotFoundError: {semantic_file} does not exist.")
